"""Load additional excluded tracking numbers from text files. Manual-only exclusions."""
import os
import logging
...
def load_exclusions_from_files(conn):
    ...
    Only loads exclusions from manually created text files.
    The automatic strike system has been disabled - exclusions are managed manually.
    """
    ...
    logger.info(f"Manual exclusions loaded: {loaded} total from text files")
    return loaded
